...
function update() {
  ...
  if (wantedScrollTop <= document.getElementById('description').scrollTop + 323) {
    doScroll = false;
  }
}
  ...
  if (Math.round(vx / (canvas.width / 12)) > 11) vx = 11 * (canvas.width / 12);
  if (Math.round(vx / (canvas.width / 12)) < 1)  vx =      (canvas.width / 12);
  if (Math.round(vy / (canvas.width / 12)) > 11) vy = 11 * (canvas.width / 12);
  if (Math.round(vy / (canvas.width / 12)) < 1)  vy =      (canvas.width / 12);
...
// белый круг
  ...
  for (let i = 0; i < Math.floor(dots.length / 2); i++) {
    if (document.getElementById('img' + (i + 1)).getAttribute('data-phase') == 'false') {
      ctx.beginPath();
      ctx.rect(dots[i * 2].x - 45, dots[i * 2].y - 40, 90, 20);
      ctx.shadowBlur = 5;
      ctx.fillStyle = 'white';
      ctx.fill();
    ...
    if ((i + 1) % 26 != 0) {
      ctx.fillText(letter((i * 2 + 1) % 26) + Math.floor((i * 2 + 1) / 26) + '(' + Math.round((dots[i * 2].x / (canvas.width / 12) - 6)) + '; ' + -1 * Math.round((dots[i * 2].y / (canvas.height / 12) - 6)) + ')', dots[i * 2].x, dots[i * 2].y - 25)
    } else {

      ctx.fillText(letter(25) + '(' + Math.round((dots[i * 2].x / (canvas.width / 12) - 6)) + '; ' + -1 * Math.round((dots[i * 2].y / (canvas.height / 12) - 6)) + ')', dots[i * 2].x, dots[i * 2].y - 25)
    ...
    if ((i + 2) % 26 != 0) {
      ctx.fillText(letter((i * 2 + 2) % 26) + Math.floor((i * 2 + 2) / 26) + '(' + Math.round((dots[i * 2 + 1].x / (canvas.width / 12) - 6)) + '; ' + -1 * Math.round((dots[i * 2 + 1].y / (canvas.height / 12) - 6)) + ')', dots[i * 2 + 1].x, dots[i * 2 + 1].y - 25)
    } else {
...
// текст
  ...
  ctx.font = 'bold italic 15px Consolas'
  ctx.textAlign = 'center';
  ctx.fillText('(' + Math.round((dot.x / (canvas.width / 12) - 6)) + '; ' + -1 * Math.round((dot.y / (canvas.width / 12) - 6)) + ')', dot.x, dot.y - canvas.height / 20)
  ctx.closePath();
...
document.addEventListener('keyup' , removeLastDot);

function removeLastDot(e) {

  if (e.keyCode == 8 && dots.length % 2 != 0) {
    dots.pop();
  }
}
...
function x1() {
  if (Math.round(dots[dots.length - 2].x / (canvas.width / 12) - 6) < 0) {
    return Math.round(dots[dots.length - 2].x / (canvas.width / 12) - 6);
  } else {
    return Math.round(dots[dots.length - 2].x / (canvas.width / 12) - 6);
  }
}
...
function y1() {
  if (-1 * Math.round(dots[dots.length - 2].y / (canvas.width / 12) - 6) < 0) {
    return -1 * Math.round(dots[dots.length - 2].y / (canvas.width / 12) - 6);
  } else {
    return -1 * Math.round(dots[dots.length - 2].y / (canvas.width / 12) - 6);
  }
}
...
function x2Bracket() {
    return Math.round(dots[dots.length - 1].x / (canvas.width / 12) - 6);
}

function y2Bracket() {
    return -1 * Math.round(dots[dots.length - 1].y / (canvas.width / 12) - 6);
}
...
function changeRuler() {
  if (document.getElementById('ruler').getAttribute('data-phase') == 'false') {
  document.getElementById('ruler').setAttribute('data-phase',  'true');
  document.getElementById('ruler').setAttribute('src',  'img/rulertrue.png');
} else {
  document.getElementById('ruler').setAttribute('data-phase',  'false');
  document.getElementById('ruler').setAttribute('src',  'img/rulerfalse.png');
}
}

function formula() {
  if (document.getElementById('ruler').getAttribute('data-phase') == 'false') {
  return '<h2> Довжина: </h2> $$\\sqrt{(x_2 - x_1)^2 + (y_2 - y_1)^2} = \\sqrt{(' + x2Bracket() + ' - ' + x1Bracket() + ')^2 + (' + y2Bracket() + ' - ' + y1Bracket() + ')^2} = \\sqrt{' + (Math.pow((x2Bracket() - x1()), 2) + Math.pow((y2Bracket() - y1()), 2)) + '} = ' + Math.round(Math.sqrt(Math.pow(Math.round(dots[dots.length - 1].x / (canvas.width / 12) - 6) - Math.round(dots[dots.length - 2].x / (canvas.width / 12) - 6), 2) + Math.pow(-1 * Math.round(dots[dots.length - 1].y / (canvas.width / 12) - 6) - -1 * Math.round(dots[dots.length - 2].y / (canvas.width / 12) - 6), 2)) * 100) / 100 + '$$'
} else return '';
}
